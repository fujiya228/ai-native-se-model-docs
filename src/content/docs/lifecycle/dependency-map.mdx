...
以下の図は、L1間の主要な依存関係の流れを示す。実線は順方向の依存（producer-consumer / gate）、破線はフィードバックループ（逆方向の依存）を表す。
...
| モデルファイル | 対応する内容 |
|---|---|
| `model/04d_dependency_map.md` | 依存関係表の全内容、依存の型定義 |
| `model/04a_lifecycle_l2_table.md` | 依存関係の土台となるL2分解 |
| `model/04_lifecycle.md` | ループ構造の概要 |
